...
int8_t STORAGE_GetCapacity_FS(uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
  ...
  *block_num  = sdInfo.LogBlockNbr - 1; // STORAGE_BLK_NBR;
  *block_size = sdInfo.LogBlockSize; //STORAGE_BLK_SIZ;
  return (USBD_OK);
  /* USER CODE END 3 */
}
...
int8_t STORAGE_Read_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  /* USER CODE BEGIN 6 */
	...
	while (HAL_SD_GetCardState(&hsd1) != HAL_SD_CARD_TRANSFER){
		if ((HAL_GetTick() - timeStart) >= SD_CARD_TIMOUT_MS){ //5sec
			return USBD_FAIL;
		}
	}
  return (USBD_OK);
